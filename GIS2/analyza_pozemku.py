...
#   pip install pandas openpyxl matplotlib seaborn scipy numpy folium shapely python-docx
#
...
OCE_OKRES        = "Hlavní město Praha" # NOVÉ: okres pro dynamické texty
...
# Prahové hodnoty pro detekci outlierů (IQR metoda)
OUTLIER_IQR_MULT = 2.5               # hodnoty mimo Q1-2.5*IQR nebo Q3+2.5*IQR
...
# Jednotková cena – použijeme sloupec JC_calc pokud existuje, jinak #JC [Kč/m2]
...
# Výpočet „čas v dnech" (kladné = transakce v minulosti)
df["cas_v_dnech"] = (DATE_REF - df["datum_podani"]).dt.days

# Normalizace UP – strip whitespace
if "UP" in df.columns:
...
# Pravidlo pro UP při více parcelách v jednom vkladu (metodika bod 3):
# použijeme UP s nejvyšším součtem výměry, pokud >50 %; jinak "UP smíšené"
def dominant_up(sub):
    if sub["UP"].nunique() == 1:
...
# První hodnoty pro skalární sloupce (stejné v rámci vkladu)
agg_dict = {
    "datum_podani":  "first",
    "cas_v_dnech":   "first",
    "JC":            "first",       # JC je stejná pro všechny parcely vkladu
    "vymera":        "first",       # #CELKOVA_VYMERA je stejná
    "cena_celkem":   "first",
    "ku_nazev":      "first",
    "refPoint_lat":  "first",
    "refPoint_lon":  "first",
...
# Dropneme řádky bez JC nebo výměry
...
def parse_geometry(geom_str):
    """Parsuje řetězec S-JTSK souřadnic 'X1 Y1 X2 Y2 …' na Shapely Polygon."""
    try:
        if pd.isna(geom_str) or str(geom_str).strip() == "":
            return None
...
if "geometry_posList" in df_dedup.columns:
    ...
    df_dedup["pp_index"]  = df_dedup.apply(
        ...
        if (r["pp_perim"] is not None and r["pp_perim"] > 0 and not np.isnan(r["pp_perim"]))
        else np.nan, axis=1
    )
...
print("[5] Popisné statistiky …")

def weighted_stats(series, weights=None):
    """Vrátí dict se statistikami; vážený průměr pokud jsou váhy."""
    s = series.dropna()
...
stats_cas   = weighted_stats(df_dedup["cas_v_dnech"])
stats_vym   = weighted_stats(df_dedup["vymera"],   df_dedup["vymera"])
stats_jc    = weighted_stats(df_dedup["JC"],        df_dedup["vymera"])

print(f"\n    JC [Kč/m²] – vážený průměr: {stats_jc['průměr']:,.0f}  |  medián: {stats_jc['medián']:,.0f}")
print(f"    Rozsah JC: {stats_jc['min']:,.0f} – {stats_jc['max']:,.0f} Kč/m²")

# =============================================================================
# 6. DETEKCE OUTLIERŮ (Log-normální IQR metoda)
# =============================================================================

print("[6] Detekce outlierů (Log-normální IQR metoda) …")

# Využíváme robustní metodu pro log-normální rozdělení cen nemovitostí
log_jc = np.log(df_dedup["JC"])
Q1_log = log_jc.quantile(0.25)
Q3_log = log_jc.quantile(0.75)
IQR_log = Q3_log - Q1_log
OUTLIER_LOG_MULT = 1.5 

lower_bound = np.exp(Q1_log - OUTLIER_LOG_MULT * IQR_log)
upper_bound = np.exp(Q3_log + OUTLIER_LOG_MULT * IQR_log)
...
print(f"    Použitý násobek (k) pro Log-IQR: {OUTLIER_LOG_MULT}")
print(f"    Detekováno outlierů: {len(df_outliers)}  (hranice: {lower_bound:,.0f} – {upper_bound:,.0f} Kč/m²)")
print(f"    Vkladů v čistém vzorku: {len(df_clean)}")
...
# --- K1 (čas) – log-lineární regrese: log(JC) ~ cas_v_dnech ---
df_reg_k1 = df_clean[df_clean["JC"] > 0].copy()
df_reg_k1["log_JC"] = np.log(df_reg_k1["JC"])

# OPRAVA: Ošetření malého vzorku nebo nulové variance (aby nevracelo NaN)
...
# --- K2 (lokalita) – medián JC k.ú. oceňované / medián JC k.ú. transakce ---
...
# --- K3 (typ UP) – medián JC UP oceňované / medián JC UP transakce ---
...
# --- K4 (velikost) – log-log regrese: log(JC) ~ log(vymera) ---
df_reg_k4 = df_clean[df_clean["JC"] > 0].copy()
df_reg_k4["log_JC"]   = np.log(df_reg_k4["JC"])
df_reg_k4["log_vym"]  = np.log(df_reg_k4["vymera"])

# OPRAVA: Ošetření malého vzorku nebo nulové variance pro K4 (aby nevracelo NaN)
...
df_clean.loc[~pp_valid, "K5"] = 1.0   # nelze spočítat → neutrální koeficient
...
# OPRAVA: Robustní výpočet váženého průměru (ochrana před zbytkovými NaN)
...
if valid_mask.sum() > 0:
    ...
    if pd.isna(etalon_vazeny): etalon_vazeny = 1000.0 # Tvrdý fallback pro jistotu
...
# Ošetření statistik pro tisk, kdyby nešly spočítat
...
print(f"    Std. odchylka : {etalon_std:,.0f} Kč/m²")
print(f"    Rozsah (Q1–Q3): {etalon_p25:,.0f} – {etalon_p75:,.0f} Kč/m²")
...
def prepocitej_etalon_citlivost(df_base, zmeny_params):
    """Přepočítá etalon při změně základních parametrů modelu."""
    df_tmp = df_base.copy()
    ...
    # OPRAVA: Bezpečný výpočet průměru v rámci iterace
    v_mask = jc_uprenava.notna() & df_tmp["vymera"].notna()
    if v_mask.sum() > 0:
        return np.average(jc_uprenava[v_mask], weights=df_tmp.loc[v_mask, "vymera"])
...
for k_label, p_name in param_map.items():
    ...
    # Ošetření, pokud by se vrátilo NaN
    ...
    print(f"    {k_label}: +10% parametru → {vaz_up:,.0f} | -10% parametru → {vaz_dn:,.0f} Kč/m² (rozpětí {abs(vaz_up-vaz_dn):,.0f})")
...
bins = np.logspace(np.log10(df_dedup["JC"].min()), np.log10(df_dedup["JC"].max()), 20)
...
ax.axvline(lower_bound,          color="darkred", linestyle="-.", linewidth=1.5, label=f'Log-IQR Dolní mez ({lower_bound:,.0f})')
ax.axvline(upper_bound,          color="darkred", linestyle="-.", linewidth=1.5, label=f'Log-IQR Horní mez ({upper_bound:,.0f})')
...
# 1. Ponecháme formátování POUZE pro hlavní (major) rysky
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:,.0f}"))
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:,.0f}"))

# 2. ZCELA VYPNE textové popisky u vedlejších (minor) rysek
ax.xaxis.set_minor_formatter(mticker.NullFormatter())

# 3. Mírné natočení hlavních popisků pro lepší čitelnost delších čísel
...
# Trendová linie - OPRAVA PROTI NaN a selhání regrese
...
ax.plot(x_range, m * x_range.map(pd.Timestamp.toordinal) + b,
        color="navy", linewidth=2, label="Lineární trend")

# Statistické linie (Barevně sjednoceno s histogramem)
for val, lbl, c, ls, lw in [
    (stats_jc["medián"], "Medián", "red", "--", 1.5),
    (stats_jc["průměr"], "Průměr", "green", "--", 1.5),
    (stats_jc["Q1"], "Q1", "orange", ":", 1.2),
    (stats_jc["Q3"], "Q3", "orange", ":", 1.2),
    (stats_jc["P5"], "P5", "gray", "-.", 1.0),
    (stats_jc["P95"], "P95", "gray", "-.", 1.0),
]:
    if pd.notna(val):
...
# ── 11.3 Analýza dle UP ───────────────────────────────────────────────────
up_stats = df_clean.groupby("UP")["JC"].agg(
    median="median",
    mean="mean",
    std="std",
    count="count",
...
# Ošetření NaN ve std pro errorbar
std_vals = up_stats["std"].fillna(0)
ax.errorbar(x + 0.2, up_stats["mean"], yerr=std_vals, fmt="none",
...
sc4   = ax.scatter(df_clean["vymera"], df_clean["JC"],
                   c=df_clean["JC"], cmap="RdYlGn", norm=norm4, s=60, alpha=0.8)
...
fig, ax = plt.subplots(figsize=(10, 5))
ax.scatter(df_clean["datum_podani"],    df_clean["JC"],
           color="#4472C4", s=40, label="Standardní transakce", alpha=0.7)
if len(df_outliers) > 0:
    ax.scatter(df_outliers["datum_podani"], df_outliers["JC"],
               color="red", s=80, marker="X", label="Outlier", zorder=5)
ax.axhline(lower_bound, color="orange", linestyle="--", linewidth=1.5,
           label=f"Dolní hranice ({lower_bound:,.0f})")
ax.axhline(upper_bound, color="orange", linestyle="--", linewidth=1.5,
           label=f"Horní hranice ({upper_bound:,.0f})")
ax.set_xlabel("Datum podání")
ax.set_ylabel("Jednotková cena [Kč/m²]")
ax.set_title("Identifikace extrémů v souboru dat")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:,.0f}"))
...
labels_map = {
    "K1": "K1 – Čas (Sklon regrese)", 
    "K2": "K2 – Lokalita (Medián k.ú.)",
    "K3": "K3 – Typ UP (Medián ÚP)", 
    "K4": "K4 – Velikost (Sklon regrese)", 
    "K5": "K5 – Tvar (PP Index)"
}
colors_up  = "#70AD47"
colors_dn  = "#FF0000"
...
for i, k in enumerate(tornado_order):
    ...
    # Bezpečné ověření hodnot
    if pd.isna(r["dn_vazeny"]) or pd.isna(r["up_vazeny"]):
        continue
        
    lo = min(r["dn_vazeny"], r["up_vazeny"])
    hi = max(r["dn_vazeny"], r["up_vazeny"])
...
# ULTIMÁTNÍ POJISTKA PROTI PÁDU - Garantuje, že X limity nebudou nikdy NaN/Inf
if np.isinf(min_x) or np.isinf(max_x) or pd.isna(min_x) or pd.isna(max_x):
    ...
    min_x = bezpecny_etalon * 0.8
    max_x = bezpecny_etalon * 1.2
    
if pd.isna(min_x): min_x = 800.0
if pd.isna(max_x): max_x = 1200.0

# Rozšíření limitů osy X o 25 % na obě strany
...
# ── 11.7 Statická prostorová mapa pro Word (WGS84 verze) ──────────────────
...
if not valid_gps.empty:
    gdf = gpd.GeoDataFrame(
        valid_gps,
        geometry=gpd.points_from_xy(valid_gps[LON_COL], valid_gps[LAT_COL]),
        crs="EPSG:4326"
    ).to_crs("EPSG:3857")
    ...
    gdf.plot(
        ...
        column="JC",
        cmap="RdYlGn_r",
        norm=Normalize(vmin=df_dedup["JC"].min(), vmax=df_dedup["JC"].max()),
        markersize=80,
        edgecolor="black",
        alpha=0.8,
        legend=True,
        legend_kwds={'label': "Jednotková cena [Kč/m²]", 'shrink': 0.6},
        zorder=2
    )
    ...
    oce_gdf = gpd.GeoDataFrame(
        index=[0], 
        crs="EPSG:4326", 
        geometry=[gpd.points_from_xy([OCE_LON], [OCE_LAT])[0]]
    ).to_crs("EPSG:3857")
    ...
    ax.scatter(
        ...
        edgecolors="white", 
        zorder=10, 
        label="Oceňovaná nemovitost"
    )

    ctx.add_basemap(ax, crs=gdf.crs.to_string(), source=ctx.providers.Esri.WorldImagery)

    legend_elements = [Line2D([0], [0], marker='^', color='w', label='Oceňovaná nemovitost',
                              markerfacecolor='blue', markeredgecolor='white', markersize=15)]
    ...
    print(f"    Uložena ortofoto mapa: {p}")
...
colormap = cm.LinearColormap(colors=['#d7191c', '#fdae61', '#a6d96a', '#1a9641'], 
                             vmin=jc_min, vmax=jc_max, 
                             caption='Jednotková cena [Kč/m²]')
m.add_child(colormap)

for _, row in df_dedup.iterrows():
    lat = row.get("refPoint_lat")
    lon = row.get("refPoint_lon")
    if pd.isna(lat) or pd.isna(lon):
        continue
    popup_txt = (
        f"<b>Vklad: {row['cislo_vkladu']}</b><br>"
        f"KÚ: {row.get('ku_nazev','–')}<br>"
        f"UP: {row.get('UP','–')}<br>"
        f"JC: <b>{row['JC']:,.0f} Kč/m²</b><br>"
        f"Výměra: {row['vymera']:,.0f} m²<br>"
        f"Datum: {str(row.get('datum_podani',''))[:10]}"
    )
    folium.CircleMarker(
        location=[lat, lon],
        radius=8,
        ...
        weight=1,
        fill=True,
        fill_color=colormap(row["JC"]),
        fill_opacity=0.9,
        popup=folium.Popup(popup_txt, max_width=250),
        tooltip=f"{row['JC']:,.0f} Kč/m²",
    ).add_to(m)

folium.Marker(
    location=[OCE_LAT, OCE_LON],
    popup=folium.Popup(
        f"<b>OCEŇOVANÁ NEMOVITOST</b><br>{OCE_KU}<br>UP: {OCE_UP}<br>"
        f"Výměra: {OCE_VYMERA:,.0f} m²<br>PP index: {OCE_PP_INDEX}", max_width=220),
    icon=folium.Icon(color="red", icon="home", prefix="fa"),
    tooltip="Oceňovaná nemovitost",
).add_to(m)
...
with pd.ExcelWriter(OUT_XLSX, engine="openpyxl") as writer:
    ...
    df_clean_out = df_clean[[
        "cislo_vkladu","datum_podani","ku_nazev","UP","vymera",
        "JC","K1","K2","K3","K4","K5","JC_upravena","pp_index",
    ]].copy()
    df_clean_out.columns = [
        "Číslo vkladu","Datum podání","KÚ","UP","Výměra [m²]",
        "JC [Kč/m²]","K1 čas","K2 lokalita","K3 typ UP",
        "K4 velikost","K5 tvar","JC upravená [Kč/m²]","PP index",
    ]
    df_clean_out.to_excel(writer, sheet_name="Data_Očištěná", index=False)

    stat_rows = []
    for label, s in [("Čas [dny]", stats_cas),
                     ("Výměra [m²]", stats_vym),
                     ("JC [Kč/m²]", stats_jc)]:
        stat_rows.append({"Ukazatel": label, **s})
    pd.DataFrame(stat_rows).to_excel(writer, sheet_name="Statistika", index=False)
    ...
    pd.DataFrame(coef_rows).to_excel(writer, sheet_name="Analýza a porovnávací metoda", index=False)

    if len(df_outliers) > 0:
        df_out_out = df_outliers[[
            "cislo_vkladu","datum_podani","ku_nazev","UP","vymera","JC"
        ]].copy()
        df_out_out.columns = ["Číslo vkladu","Datum","KÚ","UP","Výměra [m²]","JC [Kč/m²]"]
    else:
        df_out_out = pd.DataFrame({"Poznámka": ["Žádné outliers nebyly detekovány."]})
    ...
    pd.DataFrame(etalon_rows).to_excel(writer, sheet_name="Etalon", index=False)

    sens_rows = []
    for k, v in sensitivity_results.items():
        sens_rows.append({
            "Koeficient":    k,
            "Základ [Kč/m²]":  round(v["base_vazeny"], 0),
            "+10% [Kč/m²]":    round(v["up_vazeny"],   0),
            "-10% [Kč/m²]":    round(v["dn_vazeny"],   0),
            "Rozpětí [Kč/m²]": round(v["rozpeti"],     0),
        })
    pd.DataFrame(sens_rows).to_excel(writer, sheet_name="Citlivostní analýza", index=False)
...
Transakce s neobvyklou JC (výrazně pod nebo nad průměrem trhu) mohou odrážet prodej podílu
na nemovitosti, transakci mezi spřízněnými osobami, chybu v datech nebo jiné mimotržní vlivy.
Pro jejich identifikaci je použita robustní metoda interkvartilního rozpětí (IQR) aplikovaná
na logaritmicky transformovaná data. Tento postup plně respektuje log-normální rozdělení
tržních cen nemovitostí, čímž eliminuje metodické distorze (např. záporné spodní hranice).

Outlierem je označena transakce, jejíž jednotková cena leží mimo interval definovaný Tukeyho
hradbami (násobek {OUTLIER_LOG_MULT} interkvartilního rozpětí v log-prostoru). V aplikaci na tento
konkrétní datový soubor to představuje matematicky odvozené rozmezí přípustných jednotkových
cen od {lower_bound:,.0f} Kč/m² do {upper_bound:,.0f} Kč/m². Transakce mimo tento interval jsou
z analytického vzorku vyřazeny; jsou však transparentně zdokumentovány v samostatné části.
...
Detekce extrémních hodnot metodou IQR ({OUTLIER_IQR_MULT}·IQR) identifikovala {len(df_outliers)} vkladů
