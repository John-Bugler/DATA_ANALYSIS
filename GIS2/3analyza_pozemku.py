...
# ── 11.8 K5 - Rozložení PP Indexu (NOVÉ) ──────────────────────────────────
if not pp_valid_data.empty:
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.histplot(pp_valid_data, kde=True, color="#9DC3E6", bins=20, ax=ax)
    ax.axvline(OCE_PP_INDEX, color="red", linestyle="--", linewidth=2, label=f"Oceňovaný pozemek (PP = {OCE_PP_INDEX})")
    ax.axvline(pp_valid_data.median(), color="green", linestyle=":", linewidth=2, label=f"Medián trhu (PP = {pp_valid_data.median():.2f})")
    ax.set_title("Rozložení tvarového indexu (Polsby-Popper) v tržním vzorku")
    ax.set_xlabel("Polsby-Popper Index (1.0 = ideální kruh, blížící se k 0 = složitý/úzký tvar)")
    ax.set_ylabel("Počet vkladů")
    ax.legend()
    plt.tight_layout()
    p = os.path.join(SCRIPT_DIR, "graf_K5_tvar.png")
    fig.savefig(p, dpi=150); plt.close(fig)
    GRAPH_FILES["K5_tvar"] = p
    print(f"    Uložen Graf K5 (Tvar): {p}")


# ── 11.9 Statická prostorová mapa pro Word (WGS84 verze) ──────────────────
...
# ── 11.10 Prostorová interpolace cen (Heatmap) ──────────────────────────────
...
if not valid_gps_clean.empty and len(valid_gps_clean) > 4:
    try:
        ...
        # Vykreslení contour plot (Heatmapa)
...
# --- ČÁST 1: TEXT PRO BOD 2.1 ---
metodika_text_1 = """
2.1  Tržní přístup dle IVS a "White-Box" model
...
než kolik by stála koupě srovnatelné alternativní nemovitosti (princip substituce). 

Pro zamezení subjektivního zkreslení (tzv. "Black-Box" oceňování) je v této analýze použit plně 
transparentní statistický aparát. Vlivy vlastností pozemků jsou izolovány na bázi matematických 
funkcí (regresní analýza pro spojité proměnné a robustní mediánové poměry pro kategoriální proměnné). 
Každý vypočtený koeficient vychází výlučně ze samotných datových bodů předloženého vzorku.
""".strip()
add_paragraph(doc, metodika_text_1)
doc.add_paragraph("")

# --- ČÁST 2: DYNAMICKÝ BOD 2.2 (MAPA A TABULKA) ---
...
for _, row in df_raw_table.iterrows():
    ...
    # --- parc.č. s URL ---
    cell_parc = r_cells[4]
    cell_parc.text = "" # Vyčistíme buňku
...
# --- ČÁST 3: ZBYTEK METODICKÉHO TEXTU (2.3 AŽ 2.7) ---
metodika_text_2 = f"""
...
Tvar pozemku výrazně ovlivňuje jeho využitelnost a tím i obvyklou cenu. Geometrie parcely se 
v modelu kvantifikuje bez subjektivního odhadu pomocí tzv. Polsby-Popper indexu (PP). 
Index dává do poměru reálnou plochu polygonu (A) vůči jeho obvodu (P):

    PP = (4π · A) / P²

Hodnota PP = 1,0 odpovídá dokonalému kruhu. Hodnota blížící se 0 signalizuje extrémně úzký, 
dlouhý nebo zubatý (obtížně využitelný) tvar. Pro výpočet PP indexu jsou exaktně čteny S-JTSK 
souřadnice z katastrálních polygonů (sloupec „geometry_posList").
...
mohou odrážet prodej podílu na nemovitosti, transakci mezi spřízněnými osobami, nebo jiný mimotržní vliv.

Pro zajištění objektivity je aplikována Standardní metoda interkvartilního rozpětí (Tukeyho IQR) přímo 
na lineární data jednotkových cen. Vypočtená spodní hranice je z důvodu silné pravostranné asymetrie 
cen nemovitostí zespodu limitována na úroveň 5. percentilu, aby nedocházelo k vyřazování přirozeně 
levných, ale platných pozemků. Transakce ležící mimo tento matematicky definovaný interval (od {lower_bound:,.0f} Kč/m² 
do {upper_bound:,.0f} Kč/m²) se ze statistického posouzení vyřazují.

2.6  Základní filozofie korekčních koeficientů (K1–K5)

Cílem koeficientů je eliminovat rozdíly mezi srovnávacím vzorkem a naší oceňovanou nemovitostí.
Všechny koeficienty (čas, lokalita, účel, velikost, tvar) jsou po matematickém výpočtu bezpečnostně 
oříznuty na povolený interval [0,50; 2,00], čímž je analyticky zabráněno neúměrným distorzím ceny, 
pokud by tržní regresní křivka vykazovala lokální anomálii.

2.7  Výpočet upraveného JC a ETALONU

Upravená JC pro každou transakci vznikne součinem její původní JC a všech vypočtených koeficientů:
JC_upravená = JC · K1 · K2 · K3 · K4 · K5

ETALON JC je následně stanoven jako vážený průměr upravených cen (kde váhou je celková výměra vkladu), 
čímž plošně rozsáhlejší (a tedy ekonomicky signifikantnější) transakce získávají adekvátní vliv na výsledek.
""".strip()
...
if len(df_outliers) > 0:
    doc.add_paragraph("")
    add_paragraph(doc, "Přehled identifikovaných extrémů (Vyloučeno z výpočtů):", bold=True)
...
pozemků v daném vkladu) poskytuje robustní odhad průměrné tržní cenové hladiny před aplikací 
...
stats_tbl_df = pd.DataFrame([
    ["Vážený průměr (Hrubý)", f"{stats_jc['průměr']:,.0f} Kč/m²"],
...
doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("Graf 2: Histogram jednotkových cen (Zdravý trh)").alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("")

# ── 5. Odvození koeficientů ──────────────────────────────────────────────────
add_heading(doc, "5. Odvození a exaktní zdůvodnění korekčních koeficientů K1–K5", 1)

# Dynamický výpočet pro K1
rocni_zmena_k1 = (np.exp(slope_k1 * 365) - 1) * 100
slovo_zmeny = "růst" if rocni_zmena_k1 > 0 else "pokles"

k1_text = f"""
K1 – Korekce na čas (Vývoj trhu)

Ceny nemovitostí se v čase přirozeně mění (inflace, vývoj úrokových sazeb, poptávka). Pro zachycení 
tohoto fenoménu byla provedena log-lineární regrese v závislosti na kalendářních dnech od data ocenění. 
Byla zvolena log-lineární křivka namísto běžné lineární, neboť ceny na trhu se standardně úročí 
(rostou či klesají o procenta), nikoliv o pevnou absolutní částku (Kč).

Z odhadnutého regresního sklonu (β₁ = {slope_k1:.6f}) lze matematicky odvodit, že zkoumaný datový 
vzorek v čase vykazuje průměrný roční {slovo_zmeny} cen o {abs(rocni_zmena_k1):.2f} %. Koeficient K1 pro každou historickou 
transakci tuto procentuální změnu zohledňuje a posunuje původní cenu na úroveň k datu ocenění.
""".strip()
...
k2_text = f"""
K2 – Korekce na lokalitu (Katastrální území)

Lokalita je kritickým determinantem hodnoty pozemku. Koeficient K2 se v tomto modelu počítá 
jako podíl mediánu JC v katastrálním území oceňované nemovitosti ({OCE_KU}) a mediánu JC dané 
srovnávací transakce. 

Pro stanovení hladiny byl pro obě skupiny záměrně využit medián namísto aritmetického průměru. 
Průměr by mohl být silně vychýlen lokálními extrémy (např. ojedinělý prodej prémiové parcely na okraji KÚ), 
zatímco medián přesně identifikuje střední "obvyklou" hodnotu daného území. Tento poměrový postup 
objektivně převede cenovou hladinu cizího KÚ na hladinu KÚ {OCE_KU}.

Vypočtený medián JC pro cílové k.ú. {OCE_KU}: {median_oce_ku:,.0f} Kč/m².
""".strip()
...
Územní plán přímo determinuje stavební i hospodářskou využitelnost pozemku, což radikálně formuje 
jeho cenu. Obdobně jako u lokality je pro koeficient K3 využit podíl mediánových cen. Do čitatele 
vstupuje medián ceny pozemků s funkčním využitím {OCE_UP} a do jmenovatele medián JC plochy u 
porovnávané transakce.

Vypočtený medián JC pro funkční využití {OCE_UP}: {median_oce_up:,.0f} Kč/m².
""".strip()
...
doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("Graf 4: Porovnání mediánů JC dle územního plánu (Korekční základna)").alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("")

# Dynamický text pro K4
k4_text = f"""
K4 – Korekce na velikost (Diskont na velikost)

V ekonomii nemovitostí platí fenomén klesajícího mezního užitku – s rostoucí výměrou pozemku zpravidla 
klesá jeho jednotková cena, protože celková absolutní cena naráží na limity kupní síly na trhu. 
K objektivnímu podchycení tohoto jevu model využívá tzv. log-log (mocninnou) regresi. Tato funkce 
pracuje s předpokladem konstantní elasticity, což znamená, že konkrétní procentuální zvětšení 
výměry vyvolá specifickou procentuální změnu ceny, nezávisle na tom, zda jsme u malých nebo obřích ploch.

{'Záporná hodnota regresního sklonu (elasticita β₄ = ' + str(round(slope_k4, 4)) + ') tento diskont na velikost v daném datovém souboru statisticky potvrzuje.' if slope_k4 < 0 else 'Kladná nebo nulová hodnota regresního sklonu (elasticita β₄ = ' + str(round(slope_k4, 4)) + ') naznačuje, že u tohoto konkrétního typu pozemků nebyl diskont na velikost na trhu matematicky prokázán, nebo dokonce působí prémie za celistvost plochy.'}
Na základě zjištěné elasticity byl stanoven korekční převod každé transakce k oceňované výměře {OCE_VYMERA:,.0f} m².
""".strip()
...
doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("Graf 5: Závislost jednotkové ceny na výměře s proloženým log-log trendem").alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("")

# Dynamický výpočet a text pro K5
if not pp_valid_data.empty:
    pp_med = pp_valid_data.median()
    if OCE_PP_INDEX > pp_med:
        tvar_komentar = f"nad úrovní tržního mediánu (trh = {pp_med:.2f}). Je tedy kompaktnější a teoreticky lépe využitelný, což matematicky vede k vyššímu než neutrálnímu převodnímu koeficientu"
    else:
        tvar_komentar = f"pod úrovní tržního mediánu (trh = {pp_med:.2f}). Vykazuje složitější nebo protáhlejší geometrii než průměr trhu, což matematicky vede k penalizaci prostřednictvím koeficientu"
else:
    tvar_komentar = "nelze analyzovat z důvodu absence geometrických dat v převážné části vzorku"

k5_text = f"""
K5 – Korekce na tvar pozemku (Polsby-Popper index)

Tvar parcely definuje její využitelnost (odstupy staveb od hranic, hluchá místa v rozích, špatná průjezdnost). 
Geometrická komplexita je objektivizována výše zmíněným Polsby-Popper (PP) indexem. Vzorec K5 = PP_oceňované / PP_transakce 
je přímým poměrovým ukazatelem – předpokládá, že kompaktnost parcely přímo a lineárně ovlivňuje její užitek.

Oceňovaná nemovitost má PP index tvaru stanoven na hodnotu {OCE_PP_INDEX}. Ve srovnání se shromážděným tržním 
vzorkem (viz histogram níže) se oceňovaný pozemek nachází {tvar_komentar}.
""".strip()
...
if "K5_tvar" in GRAPH_FILES:
    ...
    doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("Graf 6: Histogram tvarového rozložení trhu (Polsby-Popper index)").alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("")
...
v dané lokalitě a čase, po exaktní eliminaci statisticky atypických transakcí a po důsledné
matematické korekci na prokazatelné rozdíly v čase, lokalitě, typu, velikosti a tvaru pozemků. 

Výsledná hodnota slouží jako klíčový výchozí parametr pro individuální ocenění konkrétního pozemku,
při němž budou v rámci samostatného znaleckého postupu zohledněny případné další specifické a lokální 
vlastnosti nespecifikovatelné ve velkých datových setech (např. sklonitost svahu, věcná břemena, znečištění atd.).
""".strip()
...
add_heading(doc, "7. Citlivostní analýza modelu", 1)

citlivost_text = f"""
Znalec v rámci ověřování validity výsledku provedl citlivostní analýzu. Analýza matematicky testuje 
robustnost výsledného ETALONU JC vůči izolovaným změnám hlavních statistických parametrů (sklonů regresí a 
mediánů) pro koeficienty K1–K5. Každý parametr byl ceteris paribus (za nezměněných ostatních podmínek) 
vychýlen o ±{int(SENSITIVITY_DELTA*100)} %.

Tento postup exaktně identifikuje "úzká hrdla" modelu – ukazuje, který ze statistických předpokladů 
má absolutně největší dopad na výslednou cenu.
""".strip()
...
param_desc = {
    ...
    "K4": "Sklon log-log regrese výměry (elasticita slevy na velikost)",
...
add_paragraph(doc, f"""
Výpočet citlivostní analýzy indikuje, že dominantní vliv na rozptyl konečného ETALONU JC vykazuje 
matematický předpoklad u koeficientu {nejcitlivejsi} (vzniká zde odchylka až {sensitivity_results[nejcitlivejsi]['rozpeti']:,.0f} Kč/m² 
při ±10% změně driveru). Tato hodnota vizualizovaná v tornádovém grafu (nejdelší sloupec) ukazuje největší 
míru nejistoty v datovém vzorku, naopak nejméně volatilní chování v systému vykazuje koeficient {tornado_order[-1]}.
""".strip())
doc.add_paragraph("")

doc.add_picture(GRAPH_FILES["tornado"], width=Inches(6))
doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph("Graf 7: Tornádový graf citlivostní analýzy ETALONU JC").alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_page_break()
...
Níže je uvedeno statistické srovnání této podmnožiny s celkovým trhem, které empiricky verifikuje úroveň korekčního koeficientu K3 (Korekce na typ ÚP). Rozptyl cen je vizualizován formou krabicového grafu (Boxplot), který jasně ohraničuje kvartilová rozpětí obou skupin.
    ...
    if "podmnozina_up" in GRAPH_FILES:
        ...
        doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
        doc.add_paragraph("Graf 8: Boxplot - Porovnání rozptylu JC sledovaného územního plánu").alignment = WD_ALIGN_PARAGRAPH.CENTER
else:
